...
const EditEmployee: React.FC = () => {
  ...
  const fetchPenempatan = async () => {
    try {
      if (!userData) {
        // console.warn("User data belum tersedia."); // Debug jika userData belum tersedia
        return;
      }
  
      let apiUrl = API_CUSTOMERS;
  
      // Jika level bukan 2, tambahkan filter berdasarkan level
      if (userData.level != "2") {
      ...
      // Tangani respons baik berupa array atau objek tunggal
      ...
      console.error("Error fetching penempatan:", error.response?.data || error.message); // Debug error
      Swal.fire("Error!", "Gagal memuat data penempatan.", "error");
    }
  };
  
  // Panggil fetchPenempatan hanya setelah userData tersedia
  useEffect(() => {
    if (userData) {
      fetchPenempatan();
    }
  ...
  const handleUpdate = async () => {
    ...
    formData.append("customer_id", formFields.id_area_patroli);
            ...
            (position) => {
              ...
              canvas.toBlob((blob) => {
                if (blob) {
                  ...
                  const fileName = `Update_Employees_${randomString}_${formattedDate.replace(/[: ]/g, "_")}.jpg`; // Nama file final
            ...
            (error) => {
              ...
              canvas.toBlob((blob) => {
                if (blob) {
                  ...
                  const fileName = `Update_Employees_${randomString}_${formattedDate.replace(/[: ]/g, "_")}.jpg`;
                  const file = new File([blob], fileName, { type: "image/jpeg" });
