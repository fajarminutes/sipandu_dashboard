...
const [dateTime, setDateTime] = useState(null);
            ...
            (position) => {
              ...
              canvas.toBlob((blob) => {
                if (blob) {
                  ...
                  const fileName = `Create_Handover_${randomString}_${formattedDate.replace(/[: ]/g, "_")}.jpg`; // Nama file final
            ...
            (error) => {
              ...
              canvas.toBlob((blob) => {
                if (blob) {
                  ...
                  const fileName = `Create_Handover_${randomString}_${formattedDate.replace(/[: ]/g, "_")}.jpg`;
                  const file = new File([blob], fileName, { type: "image/jpeg" });
  ...
  const handleSave = async () => {
    ...
    try {
      await axios.post(API_HANDOVER, formData, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      });
      Swal.fire("Berhasil!", "Data serah terima berhasil disimpan!", "success");
      navigate("/buku/serahterima");
    } catch (error) {
      Swal.fire("Error!", "Gagal menyimpan data serah terima.", "error");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    ...
    <div className="max-w-4xl mx-auto bg-white shadow-md rounded-lg p-8">
      <h1 className="text-3xl font-bold text-center mb-6">Form Tambah Serah Terima</h1>
