...
const CreateKendaraan: React.FC = () => {
              ...
              (position) => {
                ...
                canvas.toBlob((blob) => {
                  if (blob) {
                    ...
                    const fileName = `Create_Kendaraan_${randomString}_${formattedDate.replace(/[: ]/g, "_")}.jpg`; // Nama file final
              ...
              (error) => {
                ...
                canvas.toBlob((blob) => {
                  if (blob) {
                    ...
                    const fileName = `Create_Kendaraan_${randomString}_${formattedDate.replace(/[: ]/g, "_")}.jpg`;
                    const file = new File([blob], fileName, { type: "image/jpeg" });
  ...
  const validateForm = () => {
    const { vehicle_book_date, id_area, id, foto } = formFields;
    if (!vehicle_book_date || !id_area || !id || !foto) {
  ...
  const handleSave = async () => {
    ...
    try {
      await axios.post(API_VEHICLE_BOOK, formData, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      });
      Swal.fire("Berhasil!", "Data kendaraan berhasil disimpan!", "success");
      navigate("/buku/kendaraan/kendaraan");
    } catch (error) {
      Swal.fire("Error!", "Gagal menyimpan data kendaraan.", "error");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
   <>
    <div className="max-w-4xl mx-auto bg-white shadow-md rounded-lg p-8">
      <h1 className="text-3xl font-bold text-center mb-6">Form Tambah Kendaraan</h1>
