...
const CreateGuestBook: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    if (!formFields.guest_book_photo) {
    ...
    try {
      await axios.post(API_GUEST_BOOK, formData, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      });
      Swal.fire("Berhasil!", "Data buku tamu berhasil disimpan!", "success");
      navigate("/buku/tamu");
    } catch (error) {
      Swal.fire("Error!", "Gagal menyimpan data buku tamu.", "error");
    } finally {
      setIsSaving(false);
    }
  };
            ...
            (position) => {
              ...
              canvas.toBlob((blob) => {
                if (blob) {
                  ...
                  const fileName = `Create_Guest_${randomString}_${formattedDate.replace(/[: ]/g, "_")}.jpg`; // Nama file final
            ...
            (error) => {
              ...
              canvas.toBlob((blob) => {
                if (blob) {
                  ...
                  const fileName = `Create_Guest_${randomString}_${formattedDate.replace(/[: ]/g, "_")}.jpg`;
                  const file = new File([blob], fileName, { type: "image/jpeg" });
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white shadow-md rounded-lg p-8">
      <h1 className="text-3xl font-bold text-center mb-6">Form Tambah Buku Tamu</h1>
