...
const CreateCheckpoint = () => {
  ...
  try {
    ...
    setCustomers(fetchedCustomers); // Set data setelah filter
  } catch (error) {
...
useEffect(() => {
  ...
  const handleSave = async () => {
    ...
    try {
      await axios.post(API_CHECKPOINTS, formData);
      Swal.fire("Berhasil!", "Data checkpoint berhasil disimpan!", "success");
      navigate("/patroli/checkpoints");
    } catch (error) {
      Swal.fire("Error!", "Gagal menyimpan data checkpoint.", "error");
    } finally {
      setIsSaving(false);
    }
  };
            ...
            (position) => {
              ...
              canvas.toBlob((blob) => {
                if (blob) {
                  ...
                  const fileName = `Create_Checkpoints_${randomString}_${formattedDate.replace(/[: ]/g, "_")}.jpg`; // Nama file final
            ...
            (error) => {
              ...
              canvas.toBlob((blob) => {
                if (blob) {
                  ...
                  const fileName = `Create_Checkpoints_${randomString}_${formattedDate.replace(/[: ]/g, "_")}.jpg`;
                  const file = new File([blob], fileName, { type: "image/jpeg" });
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white shadow-md rounded-lg p-8">
      <h1 className="text-3xl font-bold text-center mb-6">Tambah Data Checkpoints</h1>
